 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	...
	ST_NO_BOTS = 0;				//INICIALIZA FLAGS DE BOTÕES
 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	while(1)
	{
		...
		{	
			if (--FILTRO1 == 0)		//TERMINOU FILTRO DE BOTÃO?
			{
				if (!ST_BT1)		//AÇÃO DO BOTÃO FOI EXECUTADA?
				{
					...
					ST_NO_BOTS = 0;	//LIMPA A FLAG DE NENHUM BOTÃO PRESSIONADO
				}
			}		
			continue;
		}		
		...
		if(!ST_NO_BOTS)				//NENHUM BOTÃO PRESSIONADO
		{
			tela_principal();		//IMPRIME A TELA PRINCIPAL
			ST_NO_BOTS = 1;			//LIMPA FLAG DE NENHUM BOTÃO 
		}
