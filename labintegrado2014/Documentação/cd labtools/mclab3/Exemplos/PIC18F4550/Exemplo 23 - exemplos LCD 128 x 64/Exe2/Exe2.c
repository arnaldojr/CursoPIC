 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/*
   Este software está preparado para carregar as informações correspondentes 
   a uma imagem, 128x64 pixels monocromática, armazenada em memória flash
   Uma rotina de carregamento é executada para demonstrar este recurso.
   Nota: a imagem foi dividida em 4 bancos de 256 bytes (limitação do 
   compilador para endereçamento direto à memória flash)
*/
...
void carrega_imagem(void)
{
	...
	for (y=0;y<8;y++)					// Varredura vertical do LCD
	{
		...
		{
			if ( y<2 )					// Limitação do compilador força divisão 
